...
export function CompanyForm({
  ...
  return (
    <div className="space-y-3 max-w-lg">
      ...
      <label className="block text-sm text-slate-200">
        担当者メール
        <input className={inputClassName} value={form.contactEmail ?? ""} onChange={(e) => setForm({ ...form, contactEmail: e.target.value })} />
      </label>
      <label className="block text-sm text-slate-200">
        備考
        <textarea className={`${inputClassName} min-h-[80px]`} value={form.notes ?? ""} onChange={(e) => setForm({ ...form, notes: e.target.value })} />
      </label>
