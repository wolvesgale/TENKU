...
export default function SupervisorDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    fetch("/api/v1/supervisors").then((r) => r.json()).then((res) => {
  ...
  return (
    <div className="space-y-4 p-4">
      <h1 className="text-xl font-semibold">監理団体 詳細</h1>
      ...
      <label className="text-sm text-slate-200 block">
        ...
        <select
          ...
          value={org.supervisorType ?? ""}
          onChange={(e) => setOrg({ ...org, supervisorType: e.target.value })}
        >
